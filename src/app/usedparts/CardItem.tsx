...
// const CardItem: React.FC<SectionProps> = ({
//   title,
//   subtitle,
//   products,
//   onExplore,
  ...
  return (
    <div className="px-6 sm:px-6 md:px-10 lg:px-12">
      <div className="my-8">
        ...
        <Wrapper className="max-sm:mx-0 max-sm:pl-0 max-sm:pr-0">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 max-sm:gap-[0.5rem] ">
            {products.map((product) => (
              <div
                key={product.id}
                onClick={() => handleCardClick(product.id)}
                className={`dark:bg-black dark:text-white p-1 max-sm:w-[100%]  shadow-md rounded-lg overflow-hidden border border-gray-200 transition-transform duration-500 ease-in-out ${
                  selectedCard === product.id
                    ? "shadow-xl shadow-white bg-white"
                    : "shadow-md shadow-gray-200"
                }`}
              >
                <div className="relative w-full h-40 max-sm:h-[7rem]">
                  <Image
                    src={product.imageUrl}
                    alt={product.name}
                    layout="fill"
