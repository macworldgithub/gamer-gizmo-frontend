...
const PcInspectionForm = () => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        setLocations(
          response.data.data.map((loc: { name: string }) => loc.name)
        );
      } catch (error) {
  ...
  return (
    <div className="w-full mt-2 mx-auto p-6 bg-white dark:bg-gray-900 shadow-lg rounded-lg">
      ...
      {/* Buttons */}
      <div className="mt-4">
        <button className="w-full bg-secondaryColorLight text-white py-2 rounded-md hover:bg-gray-2-">
          Book Inspection
        </button>
      </div>
    </div>
