...
const TopChats: React.FC<Props> = ({ token, limit = 10 }) => {
  ...
  return (
    <div className="max-w-2xl max-md:max-w-7xl mx-auto px-4 ">
      ...
      {chats.length > 0 ? (
        <div className="space-y-4">
          {chats.map((chat, index) => {
            ...
            return (
              <div key={`chat-${chat.id}-container`}>
                <div
                ...
                {/* ✅ Insert Ad after every 3 chats */}
                {shouldRenderAd && (
                  <div className="mt-2 max-sm:w-[95%]">
                    <LiveAdSection
                      key={`ad-${adId}`}
                      category="Community"
                      adId={adId}
                      className="flex-1 md:h-52 max-md:h-40"
                    />
                  </div>
                )}
              </div>
