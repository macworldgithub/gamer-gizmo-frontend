...
const categories = [
  ...
  { id: 8, image: "/images/newDesktops.png", title: "New Desktops" },
];
...
const getCategoryLink = (category: string): string => {
  ...
  const categoryName = (Object.keys(categoryMap) as (keyof typeof categoryMap)[])
    .find((key) => category.includes(key));

  return categoryName ? `/${categoryMap[categoryName]}?condition=${condition}` : "#";
};
      ...
      <div className="w-full flex justify-center py-10 h-auto md:hidden">
        <div className="w-full grid grid-cols-3 gap-4 dark:border shadow-lg bg-[#FFFFFF] rounded-lg p-6 dark:bg-black dark:border-[#6345ED] sm:grid-cols-3">
          {categories.map((category) => (
            <Link key={category.id} href={getCategoryLink(category.title)} className="flex flex-col items-center text-center rounded-lg hover:shadow-md transition-shadow bg-white dark:bg-black">
              <Image
                src={category.image}
                alt={category.title}
                width={34}
