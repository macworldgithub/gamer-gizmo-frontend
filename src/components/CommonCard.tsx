...
const CommonCard: React.FC<CardProps> = ({ title, description, note, bannerShow, bannerCount = 1 }) => {
  return (
    // <Wrapper>
    <div>
      ...
      {bannerShow &&
        Array.from({ length: bannerCount }).map((_, index) => (
          <div key={index} className="border mt-2 h-32 dark:text-white border-pinkishBorder flex flex-col items-center justify-center">
            <p className="font-bold">Boost Your Brand Visibility</p>
            <p className="">Advertise with gamergizmo today!</p>
          </div>
        ))}
