...
//                 className={`flex-none dark:bg-black dark:text-white shadow-md rounded-lg overflow-hidden border border-gray-200 w-[55%] max-sm:w-[35%] sm:w-[40%] md:w-[35%] lg:w-[18.5%] transition-transform duration-500 ease-in-out ${
...
const PopularItemSection: React.FC<SectionProps> = ({
  ...
  const handleCardClick = (productId: number, index: number) => {
    ...
    if (scrollContainerRef.current) {
      ...
      const cardWidth = container.scrollWidth / products.length; // Calculate card width dynamically
        ...
        <Wrapper className="max-sm:mx-0 max-sm:pl-0 max-sm:pr-0">
          ...
          >
            {products.map((product, index) => (
              ...
              >
                <div className="relative w-full h-40 max-sm:h-[4rem]">
                  <Image
                    src={product.imageUrl}
                    alt={product.name}
                    layout="fill"
