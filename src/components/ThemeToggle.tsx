...
const ThemeToggle = () => {
    ...
    return (
        <div
            className="relative  md:w-[5rem] lg:w-[8rem] md:h-[2rem] lg:h-[3rem] bg-white rounded-full  flex justify-between items-center  p-1 max-md:my-6 cursor-pointer shadow-md shadow-blue-500/50" onClick={handleToggle}

        >
            {/* Icons */}
            <div className="absolute md:left-[0.8rem] lg:left-[1.5rem] max-md:left-[1rem] flex items-center space-x-12 w-36">
                <FontAwesomeIcon icon={faSun} className="z-10 " color="#000000" />
                {/* color={isDarkMode ? 'white' : 'black'} */}
            </div>
            <div className="absolute md:left-[3rem] lg:left-[65%] max-md:left-[3.7rem] flex max-lg:items-center  w-[8rem]">
                <FontAwesomeIcon icon={faMoon} className="z-10" color={`${theme === "night" ? "#ffffff" : "#000000"}`} />
                {/* style={{ color: isDarkMode ? 'white' : 'black' }} */}
            </div>
            <div
                className={`lg:w-[3.5rem] md:w-[2rem] max-md:w-[2.5rem] max-md:h-[1.7rem] md:h-[1.5rem] lg:h-[2.2rem] bg-custom-gradient rounded-full transform transition-all duration-300 ${theme === "night" ? "translate-x-[100%]" : "translate-x-0"
                    }`}
            />
        </div>
    );
